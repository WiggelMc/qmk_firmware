...
const uint16_t chord_definitions[] = {
    /* 00000 00000 L- */ [0x0] = 1, // +S
    /* 11111 11111 L1 */ [0x1] = 1,
};
...
bool read_input_uint8m_gen(chord_t chord, uint8m_t *out_data) {
    ...
    out_data->byte = 0
        | (IS_BIT_SET_F(chord, 0) << 1)
        | (IS_BIT_SET_F(chord, 1) << 2)
        | (IS_BIT_SET_F(chord, 2) << 3)
        | (IS_BIT_SET_F(chord, 3) << 4)
        | (IS_BIT_SET_F(chord, 4) << 5)
        | (IS_BIT_SET_F(chord, 5) << 6)
        | (IS_BIT_SET_F(chord, 6) << 7)
        | (IS_BIT_SET_F(chord, 7) << 8)
        ;
...
bool is_chord_noop_gen(chord_t chord) {
    ...
    return false
        || MASK_CMP(chord, 0x03FF, 0x03FF) // 11111 11111
...
bool is_chord_cancel_gen(chord_t chord) {
    ...
    return false
        || MASK_CMP(chord, 0x0001, 0x03FF) // 00000 00001
        ;
}
bool is_chord_zero_gen(chord_t chord) {
    ...
    return false
        || MASK_CMP(chord, 0x0011, 0x001F) // ----- 10001
        ;
}
